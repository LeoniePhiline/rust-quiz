# Construction Site @adotinthevoid @jdonszelmann @Victoronz @GoldsteinE @WaffleLapkin @BoxyUwU @scrabsha
...
It's also interesting to see what happens when the structs are resolved through type aliases:

```rust
{{#include ../../code/examples/misc_9_4.rs}}

{{#include ../../code/examples/stderr/misc_9_4.stderr}}
```

`type` items create aliases in the type namespace only. When resolving `TypeStruct {}`, the compiler looks for `TypeStruct` in the type namespace, and successfully resolves it to `Struct`. `TypeTuple()` and `TypeUnit` are a function call and the implicit constant representing `TypeUnit`, respectively. They both live in the value namespace. When resolving them, the compiler looks up for entries in the value namespace. Since it does not find any, it triggers two errors.
</details>
